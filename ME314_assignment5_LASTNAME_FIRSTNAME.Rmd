...
```{r}
mpg01_alt <- rep(0, nrow(Auto))
mpg01_alt[Auto$mpg > median(Auto$mpg)] <- 1
...
boxplot(x = Auto$mpg01, y = Auto$cylinders)
plot(x = factor(Auto$mpg01), y = Auto$cylinders)
plot(x = jitter(Auto$mpg01), y = jitter(Auto$cylinders))
...
```{r}
set.seed(294)
Auto$set <- sample(c("train", "test"), size = nrow(Auto), replace = TRUE)

Auto_train <- subset(Auto, set == "train")
Auto_test <- subset(Auto, set == "test")
```
